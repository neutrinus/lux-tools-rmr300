...
**Faza 1: Pattern test (Eksperyment 1)** — NAJWAŻNIEJSZY
- Flash z pinami (33,18,32) + pattern generator
- 48 kroków × 3s = ~2.5 min testu
- Da nam pełne mapowanie bitów
- Jeśli wyświetlacz reaguje na wzorce → piny poprawne, trzeba tylko zmapować bity
- **Koszt:** 1 flash (~6 min) + 3 min obserwacji

**Faza 2: GPIO5 jako OE (Eksperyment 2)**
- Jeśli pattern test nie dał jasnych wyników
- Albo jeśli pattern test pokazał że coś działa ale nie wszystko
- **Koszt:** 1 flash (~6 min)
...
## 6. Proponowana implementacja — tryb `lcd_patgen`

Proponuję dodać nowy tryb diagnostyczny do komponentu: `lcd_patgen: true`.

**Zachowanie:**
1. Setup: piny display na (33,18,32) lub z konfiguracji
2. Wyłączyć normalny display refresh
3. Co 3 sekundy wysłać kolejny wzorzec z logowaniem
4. 48 kroków (24 bity × 2 fazy: common cathode + common anode)
5. Po 48 krokach → powtórzyć w pętli (lub przejść do normalnego trybu)
...
## 7. Dodatkowe obserwacje i hipotezy

### 7.1. Dlaczego "88:88" a nie wszystkie segmenty?
...
### 7.2. LCD ma 13 pinów (6 dół + 7 góra)
...
1. **Piny display: CLK=GPIO33, MOSI=GPIO18, CS=GPIO32** — potwierdzone emp. (jedyna kombinacja, która zapaliła wyświetlacz)
2. **Format danych jest nieprawidłowy** — bit order (MSB vs LSB), byte order (kolejność bajtów w kaskadzie), lub segment mapping
3. **GPIO5 może być OE** — wymaga weryfikacji, ale wyjaśniałoby rozbieżności z FW
4. **GPIO25 może być MR lub niepodłączony** — wymaga weryfikacji

### Natychmiastowy plan działania

1. **Zaimplementować `lcd_patgen`** — pattern generator w C++
2. **Flash z pinami (33,18,32) + lcd_patgen: true**
3. **Nagrać telefonem co się świeci przy każdym kroku**
4. **Na podstawie nagrań: zrekonstruować mapę bitów**
5. **Zaktualizować `char_to_segments_` i `refresh_display`**
...
1. Czy GPIO5 to OE, CLK, czy coś innego? → Eksperyment 2
2. Czy GPIO25 to MR, CS, czy NC? → Eksperyment 3
3. Jaki jest bit order (MSB vs LSB)? → Eksperyment 1 (pattern test)
4. Jaki jest byte order w kaskadzie 74HC595? → Eksperyment 6
5. Jaki jest segment mapping (który bit = który segment)? → Eksperyment 1
6. Czy wyświetlacz jest common cathode czy common anode? → Eksperyment 1 (fazy A i B)
7. Co to jest U6? → Fizyczna inspekcja lub Eksperyment 2 (GPIO5 jako OE)
8. Czy DROM SPI config (12/10) jest dla innego peryferalu? → Prawdopodobnie tak

---

## 10. Załączniki

### A. Tabela prawdu 74HC595
...
**Proponowana konfiguracja do testów:**
```yaml
snk_mower:
  display_clk: 33
  display_mosi: 18
  display_cs: 32
  lcd_patgen: true  # nowy tryb do dodania
```
